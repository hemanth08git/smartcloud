...
// IMPORTANT:
// If your API Gateway uses a stage (e.g. /prod), include it here:
// const API_BASE = "https://rvy389ztac.execute-api.us-east-1.amazonaws.com/prod";
const API_BASE = "https://rvy389ztac.execute-api.us-east-1.amazonaws.com"; // backend base URL
...
async function checkBackend() {
  try {
    ...
    backendStatusEl.textContent =
      "Backend: Offline. Start FastAPI / API at " + API_BASE;
  }
}
...
async function loadDashboard() {
  try {
    ...
    document.getElementById("highRisk").textContent =
      data.high_risk_batches ?? 0;
    document.getElementById("mediumRisk").textContent =
      data.medium_risk_batches ?? 0;
    document.getElementById("lowRisk").textContent =
      data.low_risk_batches ?? 0;
    document.getElementById("unknownRisk").textContent =
      data.unknown_risk_batches ?? 0;
  } catch (e) {
    console.error("Dashboard error", e);
  }
}
...
async function loadProducts() {
  try {
    const res = await fetch(API_BASE + "/products");
    if (!res.ok) {
      console.error("Load products response not OK", res.status);
      return;
    }
    const data = await res.json();
    productsTableBody.innerHTML = "";
    data.forEach((p) => {
      ...
      tr.innerHTML = `
        ...
        <td>${p.category || ""}</td>
        <td>
          <button onclick="editProduct(${p.id}, '${escapeQuotes(
        p.name
      )}', '${escapeQuotes(p.category || "")}', '${escapeQuotes(
        p.description || ""
      )}')">Edit</button>
          <button class="secondary" onclick="deleteProduct(${p.id})">Delete</button>
        </td>
...
// Escape quotes safely for inline onclick handlers
function escapeQuotes(str) {
  return String(str)
    .replace(/'/g, "\\'")
    .replace(/"/g, "&quot;");
...
async function loadProductsForBatches() {
  try {
    const res = await fetch(API_BASE + "/products");
    if (!res.ok) {
      console.error("Load products for batches response not OK", res.status);
      return;
    }
    const data = await res.json();
    batchProductIdSelect.innerHTML = "";
...
async function loadBatches() {
  try {
    const res = await fetch(API_BASE + "/batches");
    if (!res.ok) {
      console.error("Load batches response not OK", res.status);
      return;
    }
    const data = await res.json();
    batchesTableBody.innerHTML = "";
    data.forEach((b) => {
      const hasScore =
        b.risk_score !== null && b.risk_score !== undefined;
      const scoreText = hasScore ? ` (${b.risk_score.toFixed(1)})` : "";
      const tr = document.createElement("tr");
      tr.innerHTML = `
        <td>${b.id}</td>
        <td>${b.code}</td>
        <td>${b.product_id}</td>
        <td>${b.status}</td>
        <td>${b.risk_level || "UNKNOWN"}${scoreText}</td>
        <td>
          <button onclick="computeRisk(${b.id})">Compute Risk</button>
          <button class="secondary" onclick="deleteBatch(${b.id})">Delete</button>
        </td>
...
window.computeRisk = async function (id) {
  try {
    ...
    console.error("Compute risk error", e);
  }
};

// --------------------------
// Inspections & sensor readings
// --------------------------
const inspectionBatchIdSelect = document.getElementById("inspectionBatchId");
const sensorBatchIdSelect = document.getElementById("sensorBatchId");
const inspectionForm = document.getElementById("inspectionForm");
...
async function loadBatchesForInspections() {
  try {
    ...
    if (!res.ok) {
      console.error(
        "Load batches for inspections response not OK",
        res.status
      );
      return;
    }
...
inspectionForm.addEventListener("submit", async (e) => {
  ...
  const payload = {
    ...
    temperature: parseFloat(
      document.getElementById("inspectionTemp").value
    ),
    humidity: document.getElementById("inspectionHumidity").value
      ? parseFloat(document.getElementById("inspectionHumidity").value)
      : null,
...
sensorForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    console.error("Create sensor reading error", e);
  }
});
...
async function loadAlerts() {
  try {
    const res = await fetch(API_BASE + "/alerts");
    if (!res.ok) {
      console.error("Load alerts response not OK", res.status);
      return;
    }
    const data = await res.json();
    alertsTableBody.innerHTML = "";
  ...
  sensorAnomalyForm.addEventListener("submit", async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/sensor/analyze/batch/${batchId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      anomalyStatus.textContent = data.is_anomaly
        ? "Anomaly Detected"
        : "Normal";
      anomalyStatus.className = data.is_anomaly
        ? "badge badge-high"
        : "badge badge-low";
      ...
      console.error("Sensor anomaly error", err);
      alert("Error connecting to the analysis service.");
    }
  });
}
