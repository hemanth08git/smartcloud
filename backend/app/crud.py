...
def update_batch(db: Session, batch_id: int, batch_in: schemas.BatchUpdate) -> Optional[models.Batch]:
...
def get_inspections_for_batch(db: Session, batch_id: int) -> List[models.Inspection]:
    return db.query(models.Inspection).filter(models.Inspection.batch_id == batch_id).all()


def create_inspection(db: Session, inspection_in: schemas.InspectionCreate) -> models.Inspection:
...
# Sensor readings
def create_sensor_reading(db: Session, reading_in: schemas.SensorReadingCreate) -> models.SensorReading:
...
def get_sensor_readings_for_batch(db: Session, batch_id: int) -> List[models.SensorReading]:
    return db.query(models.SensorReading).filter(models.SensorReading.batch_id == batch_id).all()


# Alerts & dashboard
def create_alert(db: Session, batch_id: int, level: str, message: str) -> models.Alert:
...
def get_dashboard_summary(db: Session) -> schemas.DashboardSummary:
    ...
    high_risk = db.query(models.Batch).filter(models.Batch.risk_level == "HIGH").count()
    medium_risk = db.query(models.Batch).filter(models.Batch.risk_level == "MEDIUM").count()
    low_risk = db.query(models.Batch).filter(models.Batch.risk_level == "LOW").count()
    unknown_risk = db.query(models.Batch).filter(models.Batch.risk_level == "UNKNOWN").count()
...
def get_recent_sensor_readings(db: Session, batch_id: int, limit: int = 50):
    """
    Return the most recent sensor readings for a given batch.
    Assumes models.SensorReading has fields: batch_id, temperature, humidity, timestamp.
    """
    return (
