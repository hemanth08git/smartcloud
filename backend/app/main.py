...
# ==========================
# NOTE: data is kept only in RAM and resets when the server restarts.
# In a real AWS DynamoDB deployment, db_dynamo would be implemented
# with boto3 calls instead of an in-memory dict.
...
def create_inspection(
    ...
    batch = (
        ...
        .filter(models.Batch.id == inspection_in.batch_id)
        .first()
    )
    if not batch:
        raise HTTPException(status_code=404, detail="Batch not found")
...
def create_sensor_reading(
    ...
    batch = (
        db.query(models.Batch)
        .filter(models.Batch.id == reading_in.batch_id)
        .first()
    )
    if not batch:
        raise HTTPException(status_code=404, detail="Batch not found")
...
def compute_risk(batch_id: int, db: Session = Depends(get_db)):
    ...
    if result.level in ("MEDIUM", "HIGH"):
        crud.create_alert(
            ...
            level=result.level,
            message=(
                f"Batch {batch.code} has {result.level} spoilage risk. "
...
def dashboard_summary(db: Session = Depends(get_db)):
    """
    Combine relational (batches / risk) stats from SQLAlchemy with
    product counts from the Dynamo-style product store (db_dynamo).
    This keeps the dashboard in sync with the /products CRUD API.
    """
    # Get batch & risk stats from the relational DB
    base_summary = crud.get_dashboard_summary(db)

    # Products are managed via db_dynamo (DynamoDB-style store)
    total_products = len(db_dynamo.list_products())

    return schemas.DashboardSummary(
        total_products=total_products,
        total_batches=base_summary.total_batches,
        high_risk_batches=base_summary.high_risk_batches,
        medium_risk_batches=base_summary.medium_risk_batches,
        low_risk_batches=base_summary.low_risk_batches,
        unknown_risk_batches=base_summary.unknown_risk_batches,
    )
...
def analyze_batch_sensor_reading(
    ...
    if not history_rows:
        # No historical readings means we cannot compute an anomaly score yet.
        # Return a 400 so the frontend can show a user-friendly message.
        raise HTTPException(
            status_code=400,
            detail=(
                "No sensor history found for this batch. "
                "Please record some sensor readings first via the Inspections page."
            ),
        )
